...
import { NEX_PRO_INSTRUCTION, NEXA_FLASH_INSTRUCTION } from "./systemPrompts.ts";
...
function verifyApiAccess(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  if (!session) {
    ...
    const isTurnstileConfigured = Boolean(process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY);
    const newSessionId = crypto.randomBytes(32).toString("hex");
    session = {
      ip: clientIp,
      createdAt: now,
      lastActiveAt: now,
      expiresAt: now + (isTurnstileConfigured ? UNVERIFIED_SESSION_MAX_AGE_MS : VERIFIED_SESSION_MAX_AGE_MS),
      turnstileVerified: !isTurnstileConfigured,
    };
    validSessions.set(newSessionId, session);

    res.cookie("nex_pro_session", newSessionId, getSessionCookieOptions(req, isTurnstileConfigured ? UNVERIFIED_SESSION_MAX_AGE_MS : VERIFIED_SESSION_MAX_AGE_MS));
  }

  // 4. Enforce Turnstile Human Verification for protected AI Execution Endpoints (only if secret key is configured)
  const isTurnstileConfigured = Boolean(process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY);
  ...
  if (isTurnstileConfigured && isProtectedAiEndpoint && !session.turnstileVerified) {
...
app.post("/api/init-session", initSessionLimiter, (req, res) => {
  ...
  res.cookie("nex_pro_session", sessionId, getSessionCookieOptions(req, isTurnstileConfigured ? UNVERIFIED_SESSION_MAX_AGE_MS : VERIFIED_SESSION_MAX_AGE_MS));

  // Never return sessionId to JavaScript (keeps httpOnly cookie completely unreadable by XSS)
  return res.json({ ok: true, sessionActive: true, turnstileVerified: !isTurnstileConfigured });
});

// Cloudflare Turnstile Configuration Route (Returns public site key configured on server)
app.get("/api/turnstile-config", generalApiLimiter, (req, res) => {
  ...
  const isRealConfigured = Boolean(
    siteKey &&
    !siteKey.includes("0000000000000") &&
    process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY &&
    !process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY.includes("0000000000000")
  );
...
app.post("/api/chat", express.json({ limit: "8mb" }), chatRateLimiter, verifyApiAccess, async (req, res) => {
  ...
  // Assign system prompt specifically for NEX_PRO
  const isNexPro =
    requestedModel === "NEXA" ||
    requestedModel === "NEX-Pro" ||
    requestedModel === "NEX_PRO" ||
    (requestedModel !== "NEXA-FLASH" && requestedModel !== "NEX-PRO-Z");

  const defaultInstruction = isNexPro ? (NEX_PRO_INSTRUCTION || "") : (NEXA_FLASH_INSTRUCTION || "");
  const baseInstruction: string = safeCustomPrompt ? safeCustomPrompt.trim() : defaultInstruction;
  ...
  // Build OpenAI Messages (Completely blank/empty system prompt by default)
  const openAiMessages: Array<OpenAI.Chat.Completions.ChatCompletionMessageParam> = [];
  if (baseInstruction && baseInstruction.length > 0) {
  ...
  // ============================================================================
  // Model Engine Dispatcher with Resilient Multi-Model Fallback
  // ============================================================================
  try {
    let displayModel = "DEV PRO";
    let candidateModels: { provider: "xkiro" | "unorouter", id: string }[] = [];

    if (model === "NEXA-FLASH") {
      displayModel = "DEV Flash";
      if (hasImageAttachments || hasMediaAttachments) {
        displayModel += " (Vision)";
        candidateModels = [
          { provider: "xkiro", id: "mistralai/mistral-large-2512" },
          { provider: "xkiro", id: "google/gemini-2.5-pro" }, // Fallback
        ];
      } else {
        candidateModels = [
          { provider: "xkiro", id: "deepseek/deepseek-v4-flash" },
          { provider: "xkiro", id: "mistralai/mistral-large-2512" },
        ];
      }
    } else if (model === "NEX-PRO-Z") {
      displayModel = "DEV Think";
      if (hasImageAttachments || hasMediaAttachments) {
        displayModel += " (Vision)";
        candidateModels = [
          { provider: "xkiro", id: "deepseek/deepseek-v3.2" },
          { provider: "xkiro", id: "google/gemini-2.5-pro" }, // Fallback
        ];
      } else {
        candidateModels = [
          { provider: "unorouter", id: "glm-5.3-flash-search:free" },
          { provider: "xkiro", id: "deepseek/deepseek-v3.2" },
        ];
      }
    } else {
      // Default: DEV PRO
      displayModel = "DEV PRO";
      if (hasImageAttachments || hasMediaAttachments) {
        displayModel += " (Vision)";
        candidateModels = [
          { provider: "xkiro", id: "qwen/qwen3.8-max:free" },
          { provider: "xkiro", id: "openai/gpt-4o" }, // Fallback
        ];
      } else {
        candidateModels = [
          { provider: "xkiro", id: "deepseek/deepseek-v4-pro" },
          { provider: "xkiro", id: "qwen/qwen3.8-max:free" },
          { provider: "unorouter", id: "glm-5.3-flash-search:free" },
          { provider: "unorouter", id: "glm-5.3:free" },
        ];
      }
    }
    ...
    for (const candidate of candidateModels) {
      if (res.headersSent && requestCompleted) break;

      const client = candidate.provider === "xkiro" ? xkiroClient : unorouterClient;
      if (!client) {
        console.warn(`${candidate.provider} client not available, skipping ${candidate.id}`);
        continue;
      }
      ...
      try {
        if (stream) {
          ...
          const compRes = await client.chat.completions.create({
            model: candidate.id,
            messages: openAiMessages,
            temperature: safeTemperature,
          });
          ...
          if (text) {
            res.json({ text: sanitizeModelMentions(text), model: displayModel });
            requestCompleted = true;
            break;
          }
        }
      } catch (candidateErr: any) {
        console.warn(`${candidate.provider} failed for "${candidate.id}", trying next:`, sanitizeErrorMessage(candidateErr));
        lastError = candidateErr;
      }
    }

    // 3. Fallback to Google GenAI native engine if available and nothing sent yet
    if (!requestCompleted && !res.headersSent && googleAi) {
      try {
        const geminiRes = await googleAi.models.generateContent({
          ...
          config: {
            temperature: safeTemperature,
            systemInstruction: baseInstruction || undefined,
          },
        });
        ...
        if (text) {
          if (stream) {
            ...
            res.setHeader("Connection", "keep-alive");
            res.flushHeaders();
            res.write(`data: ${JSON.stringify({ text: sanitizeModelMentions(text) })}\n\n`);
            res.write("data: [DONE]\n\n");
            return res.end();
          } else {
        ...
        console.warn("Google GenAI engine fallback failed:", sanitizeErrorMessage(geminiErr));
        lastError = geminiErr;
      }
    }
    ...
    throw new Error("No AI service available at this time");
  } catch (err: any) {
...
async function startServer() {
  ...
  // Only start the server locally, don't start it on Vercel
