...
// Default Cloudflare Turnstile Sitekey fallback
...
export const CloudflareTurnstile: React.FC<CloudflareTurnstileProps> = ({
  ...
  const [activeSiteKey, setActiveSiteKey] = useState<string>(DEFAULT_SITE_KEY);
  const [isRealKey, setIsRealKey] = useState<boolean>(false);
  ...
  // Fetch real sitekey from server if configured in environment variables
  useEffect(() => {
    let isMounted = true;
    authFetch("/api/turnstile-config")
      .then((res) => res.json())
      .then((data) => {
        if (isMounted && data?.siteKey) {
          setActiveSiteKey(data.siteKey);
          if (data.isRealConfigured) {
            setIsRealKey(true);
          }
        }
      })
  ...
  }, [isDark, compact, onVerify, onError, activeSiteKey]);
  ...
  return (
    <div className={`rounded border transition-all select-none ${className} ${
      ...
      <div className="p-3.5 sm:p-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className={`w-8 h-8 rounded-xs flex items-center justify-center shrink-0 border shadow-2xs ${
            isVerified 
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {isRealKey && (
                <span className="text-[10px] px-1.5 py-0.2 rounded-xs font-mono font-bold bg-blue-500/15 text-blue-400 border border-blue-500/30 tracking-wider">
                  LIVE
                </span>
              )}
              <span className={`text-[10.5px] px-2 py-0.5 rounded-xs font-medium border ${
                isVerified
        ...
        <button
          ...
          className={`p-2 rounded-xs border text-xs font-medium transition-colors cursor-pointer shrink-0 ${
            isDark
