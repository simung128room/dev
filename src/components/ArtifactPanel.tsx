    ...
    >
      ...
      <div className="h-14 px-3 sm:px-4 bg-white/95 backdrop-blur-md border-b border-zinc-200 flex items-center justify-between gap-3 shrink-0 select-none">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          ...
          <div className="flex items-center p-0.5 bg-zinc-100 rounded border border-zinc-200 shrink-0">
            <button
              type="button"
              onClick={() => {
                setViewMode("preview");
          ...
          <div className="flex items-center gap-2 min-w-0">
            {isEditingFilename ? (
              <input
                ...
                autoFocus
                className="bg-white text-zinc-950 px-2 py-0.5 rounded-xs border border-zinc-300 font-mono text-xs focus:outline-none w-36 sm:w-48"
              />
            ) : (
              <button
                type="button"
                onClick={() => setIsEditingFilename(true)}
                className="flex items-center gap-1.5 font-sans font-semibold text-sm sm:text-[15px] text-zinc-900 hover:text-blue-600 truncate cursor-pointer tracking-tight"
            ...
            {isStreaming && (
              <span className="inline-flex items-center gap-1 text-[11px] font-thai text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-xs shrink-0 animate-pulse font-medium">
                <Sparkles className="w-3 h-3 animate-spin text-blue-600" /> กำลังเขียนสด...
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded bg-white hover:bg-zinc-50 border border-zinc-200 text-zinc-700 hover:text-zinc-900 text-xs font-sans font-medium transition-all cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded bg-zinc-900 hover:bg-black text-white text-xs font-sans font-medium transition-all cursor-pointer shadow-2xs"
          >
          ...
          {onToggleFullscreen && (
            <button
              ...
              className="p-1.5 rounded-xs text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 transition-colors cursor-pointer"
            >
      ...
      <div className="flex-1 overflow-auto relative bg-white p-4 sm:p-6 text-zinc-900">
        {viewMode === "preview" && isMarkdown ? (
          ...
          <div className="max-w-3xl mx-auto font-thai text-zinc-900 space-y-4 leading-[1.8]">
            <ReactMarkdown
              components={{
                ...
                table({ children }) {
                  return (
                    <div className="overflow-x-auto my-4 rounded border border-zinc-200 bg-white shadow-2xs">
                ...
                blockquote({ children }) {
                  return (
                    <blockquote className="border-l-4 border-blue-600 bg-blue-50/40 pl-4 py-2 italic text-zinc-800 my-4 rounded-r border-y border-r border-zinc-200/80">
                      {children}
                    </blockquote>
                  );
                },
                ...
                code({ inline, className, children, ...props }: any) {
                  if (inline) {
                    return (
                      <code className="bg-zinc-100 px-1.5 py-0.5 rounded-xs font-mono text-[13px] text-blue-700 border border-zinc-200/80">
                        {children}
                      </code>
                    );
                  }
                  return (
                    <div className="my-3 overflow-x-auto bg-zinc-50 p-3 rounded border border-zinc-200 font-mono text-[13px] text-zinc-900">
                      <code>{children}</code>
                    </div>
                  );
                },
            ...
            {isStreaming && (
              <div className="flex items-center gap-2 text-xs font-mono text-blue-600 pt-2 animate-pulse font-medium">
          ...
          <div className="w-full h-full min-h-[500px] border border-zinc-200 rounded overflow-hidden bg-white shadow-xs">
            <iframe
          ...
          <div ref={codeContainerRef} className="h-full overflow-auto font-mono text-xs sm:text-[13px] leading-relaxed">
            <div className="min-w-full inline-block pb-10">
              {lines.map((line, idx) => (
                <div key={idx} className="flex hover:bg-blue-50/40 group py-0.5">
                  <span className="w-12 shrink-0 select-none text-right pr-4 text-zinc-400 font-mono text-xs group-hover:text-zinc-600">
                    {idx + 1}
                  </span>
                  <span className="flex-1 whitespace-pre pr-4 text-zinc-900 selection:bg-blue-100">
                    {line || " "}
                  </span>
                </div>
              ))}
              ...
              {isStreaming && (
                <div className="flex items-center py-1 pl-12 text-blue-600 animate-pulse font-medium">
                  <span className="inline-block w-2 h-4 bg-blue-600 mr-2 rounded-xs" />
      ...
      <div className="h-9 px-4 bg-zinc-50 border-t border-zinc-200 flex items-center justify-between text-[11px] font-mono text-zinc-500 shrink-0 select-none">
        ...
        <div>
          {isStreaming ? (
            <span className="text-blue-600 flex items-center gap-1.5 font-thai font-medium">
              <span className="w-1.5 h-1.5 rounded-xs bg-blue-600 animate-ping" />
