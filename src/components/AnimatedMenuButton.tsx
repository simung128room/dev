...
export const AnimatedMenuButton: React.FC<AnimatedMenuButtonProps> = ({
  ...
  return (
    <button
      ...
      className={`relative w-10 h-10 flex flex-col items-center justify-center gap-[5px] p-2 rounded-full transition-all duration-200 cursor-pointer group active:scale-95 select-none focus:outline-hidden ${
        isDark
    ...
    >
      {!isOpen && (
        <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#8ab4f8] ring-2 ring-[#131314] pointer-events-none" />
      )}
      {/* Top Bar */}
      <motion.span
