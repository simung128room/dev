...
import { ArrowDown, ShieldCheck, MessageSquarePlus, MoreVertical, ChevronDown, X } from "lucide-react";
...
export default function App() {
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-[#090a0f] text-white font-sans overflow-hidden antialiased relative selection:bg-blue-600 selection:text-white">
      ...
      {/* Main Column */}
      <main className="flex-1 flex flex-col min-w-0 h-[100dvh] relative overflow-hidden bg-[#131314] text-[#e3e3e3] z-10">
        {/* Floating Toast Notification */}
        {toastMessage && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium shadow-2xl backdrop-blur-md transition-all animate-in fade-in slide-in-from-top-4 duration-200 bg-[#1e1f20]/95 border border-[#333537] text-white">
            <span
              className={`w-2 h-2 rounded-full ${
                toastMessage.type === "success" ? "bg-blue-400" : "bg-red-400"
              }`}
            />
        ...
        {/* Top Header Bar: Gemini Mobile / Web Style Header */}
        <header className="h-14 w-full flex items-center justify-between px-3 sm:px-4 z-30 shrink-0 select-none bg-[#131314]/90 backdrop-blur-md border-b border-white/[0.04]">
          {/* Left: Hamburger Button with Notification Dot + Model Selector Pill */}
          <div className="flex items-center gap-2">
            ...
            {/* Model Selector Pill (Gemini Capsule Pill: "รวดเร็ว ระดับสูง ⌄") */}
            <div className="relative group">
              <select
                className="appearance-none bg-[#282a2c] hover:bg-[#333537] border border-[#3c4043]/40 text-[#e3e3e3] font-thai text-[13.5px] font-medium pr-7.5 py-1.5 pl-3.5 rounded-full cursor-pointer transition-colors outline-none shadow-xs"
              ...
              <ChevronDown className="w-3.5 h-3.5 text-zinc-400 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none group-hover:text-zinc-200" />
            </div>
          </div>

          {/* Right: New Chat Icon Button + 3-dots Menu Button */}
          <div className="flex items-center gap-1">
            <button
              ...
              className="w-10 h-10 rounded-full hover:bg-[#282a2c] active:bg-[#333537] text-zinc-300 hover:text-white flex items-center justify-center transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              title="เมนูเพิ่มเติม / การตั้งค่า"
              className="w-10 h-10 rounded-full hover:bg-[#282a2c] active:bg-[#333537] text-zinc-300 hover:text-white flex items-center justify-center transition-colors cursor-pointer"
            >
          ...
          <div className="flex-1 flex w-full h-full min-w-0 relative overflow-hidden">
            ...
            <div
              className={`flex flex-col h-full min-w-0 transition-all duration-200 relative ${
                activeArtifact && !isArtifactFullscreen ? "w-full lg:w-1/2 xl:w-1/2 border-r border-[#1e2638]" : "w-full"
              }`}
            >
              <div
                ref={chatScrollContainerRef}
                onScroll={handleScroll}
                className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 pt-4 pb-4 space-y-4"
              >
              ...
              {showScrollBottom && (
                <button
                  ...
                  className="absolute bottom-24 right-8 z-20 w-9 h-9 flex items-center justify-center rounded bg-[#0f131d]/95 text-zinc-300 hover:text-white hover:bg-blue-600/20 border border-[#1e2638] shadow-lg transition-all cursor-pointer active:scale-90"
                >
      ...
      {/* Turnstile Human Verification Modal (No text, Seamless Resume without new chat) */}
      {isTurnstileModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
          <div
            className={`relative p-5 rounded-2xl border shadow-2xl transition-all flex flex-col items-center ${
              currentTheme.isDark
                ? "bg-[#1e1f20] border-[#333537] text-zinc-100"
          ...
          >
            <button
              ...
              title="ปิด"
              className="absolute top-2.5 right-2.5 text-zinc-400 hover:text-zinc-200 p-1 rounded-full cursor-pointer transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className={`fixed top-4 right-4 z-50 px-4 py-2.5 rounded shadow-lg font-thai text-sm text-white ${
